    ...
    fun connect() {
        ...
        if (message.command == A_STLS) {
            if (!BuildUtils.atLeast29) {
                error("Android 9 之前不支持通过 TLS 连接 adb")
            }
            ...
            if (message.command != A_AUTH && message.arg0 != ADB_AUTH_TOKEN) error("不是 A_AUTH ADB_AUTH_TOKEN")
